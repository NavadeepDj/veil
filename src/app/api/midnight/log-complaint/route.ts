...
export async function POST(request: Request) {
  ...
  try {
    const body = (await request.json()) as { complaintHash?: unknown };
    const parsed = parseHex32('complaintHash', body.complaintHash);
    if ('error' in parsed) {
      return NextResponse.json({ error: parsed.error }, { status: 400 });
    }

    const client = await getVeilClient();
    return runMidnightAction(() => client.logComplaint(parsed.bytes));
  } catch (error) {
